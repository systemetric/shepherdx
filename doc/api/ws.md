...
API, see [this page](./http.md), for the MQTT API, see further documentation
(ref later).

The Shepherd WebSocket API can be divided into three systems, specified by HTTP
endpoint:

- `/log/robot`, buffered logs from running usercode
- `/camera`, b64 encoded images from running usercode
- all other endpoints, relayed from MQTT (ref later)

## `/log/robot`

The `/log/robot` endpoint broadcasts log messages from running usercode. These
logs are buffered from the `log/robot` MQTT topic (ref later) for the current
usercode session.
...
when client buffers should be cleared, use the MQTT relay endpoint
`shepherd-run/status`, this may change in the future.
...
All other endpoints are relayed from MQTT (ref later), e.g. the endpoint
